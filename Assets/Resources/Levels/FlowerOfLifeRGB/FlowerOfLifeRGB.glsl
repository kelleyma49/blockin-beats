...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float time2r = time + 0.0f * displace;
    rgb.r += pow(calc(tx, time2r), 2.0);

    float time2g = time + 1.0f * displace;
    rgb.g += pow(calc(tx, time2g), 2.0);

    float time2b = time + 2.0f * displace;
    rgb.b += pow(calc(tx, time2b), 2.0);

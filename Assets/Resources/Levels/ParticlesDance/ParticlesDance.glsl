...
{    
    float texVal = 0.3*texture(iChannel0, vec2(0,0)).r;
    vec4 outColor = vec4(texVal,texVal,texVal,1.);
    ...
    for(float i=0.0; i<90.0 ;i++){
        ...
        float fft1 = texture(iChannel0, vec2(f1,f1)).x;  
        float r = (fft1 / 2.);
        ...
        float birghtness = 1./pow(0.001 + dist*350., 2.)*5.;
        float birghtness2 = 1./pow(0.001 + dist2*500., 2.)*10.;        
    ...
    outColor.rgb += (outColor.rgb * vec3(grid,grid,grid) * 0.6);

...
vec2 hash22(vec2 p) { 
    ...
    return sin( p*6.2831853 + iGlobalTime )*.45 + .5; 
...
void mainImage(out vec4 c, vec2 u){
    ...
    if(t<FAR){
        ...
        c.xyz = vec3(1.,1.,1.)*(hm*.8 + .2); // Applying the shading to the final color.
        
        c.xyz *= vec3(1.5,1.5,1.5)*tx; // Multiplying by the texture value and lightening.

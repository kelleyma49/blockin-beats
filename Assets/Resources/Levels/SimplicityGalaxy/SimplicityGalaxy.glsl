...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
	...
	vec2 seed = p.xy * 2.0;	
	seed = floor(vec2(seed.x * iResolution.x,seed.y * iResolution.x));
	vec3 rnd = nrand3( seed );
	float rndPow = pow(rnd.y,40.0);
	vec4 starcolor = vec4(rndPow,rndPow,rndPow,rndPow);
	
	//Second Layer
	vec2 seed2 = p2.xy * 2.0;
	seed2 = floor(seed2 * iResolution.x);
	vec3 rnd2 = nrand3( seed2 );
	float rnd2Pow = pow(rnd2.y,40.0);
	starcolor += vec4(rnd2Pow,rnd2Pow,rnd2Pow,rnd2Pow);
